...
INT_PTR CDynDialogEx::DoModal() 
{
  ...
  int cWC = MultiByteToWideChar(CP_ACP, 0, (LPSTR)(LPCTSTR)LogFont.lfFaceName, -1, NULL, 0);
  int nFontNameLen = cWC + 1;
  WCHAR *szFontName = new WCHAR[nFontNameLen];
  // Copy the std::basic_string<TCHAR>
  MultiByteToWideChar(CP_ACP, 0, (LPSTR)(LPCTSTR)LogFont.lfFaceName, -1, (LPWSTR) szFontName, cWC);
  szFontName[cWC] = 0;
  ...
  cWC = MultiByteToWideChar(CP_ACP, 0, (LPSTR)(LPCTSTR)m_strCaption, -1, NULL, 0);
  int szBoxLen = cWC + 1;
  WCHAR *szBoxCaption = new WCHAR[szBoxLen];
#ifndef _UNICODE
  // Copy the std::basic_string<TCHAR>
  MultiByteToWideChar(CP_ACP, 0, (LPSTR)(LPCTSTR)m_strCaption, -1, (LPWSTR) szBoxCaption, cWC);
#else
  _tcscpy(szBoxCaption, m_strCaption);
#endif
  szBoxCaption[cWC] = 0;
