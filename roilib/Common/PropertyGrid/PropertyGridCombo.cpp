...
void CPropertyGridCombo::AddString(std::basic_string<TCHAR> strItem)
{
...
BOOL CPropertyGridCombo::Create(DWORD dwStyle, CRect& rc, CWnd* pParent, int nId)
{
  ...
  BOOL ret = __super::CreateEx(0, AfxRegisterWndClass(CS_HREDRAW|CS_VREDRAW), _T(""), dwStyle|WS_POPUP, rc, pParent->GetParent(), nId);
...
void CPropertyGridCombo::OnPaint()
{
  ...
  for (vector<std::basic_string<TCHAR>>::iterator it = m_Items.begin(); it != m_Items.end(); ++it)
  {
    ...
    dc.DrawText(str, rcItem, DT_SINGLELINE|DT_VCENTER|DT_LEFT|DT_NOPREFIX);
    y += m_line_height;
    i++;
  }
