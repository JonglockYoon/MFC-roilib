...
void CPropertyGrid::SetTrueFalseStrings(std::basic_string<TCHAR> strTrue, std::basic_string<TCHAR> strFalse)
{
  m_strTrue = strTrue;
  m_strFalse = strFalse;
}

void CPropertyGrid::SetOkCancelStrings(std::basic_string<TCHAR> strOk, std::basic_string<TCHAR> strCancel)
{
  m_strOk = strOk;
  m_strCancel = strCancel;
}

void CPropertyGrid::SetDateTimeStrings(std::basic_string<TCHAR> strDate, std::basic_string<TCHAR> strTime)
{
  m_strDate = strDate;
  m_strTime = strTime;
}

void CPropertyGrid::SetUndefinedString(std::basic_string<TCHAR> strUndefined)
{
  m_strUndefined = strUndefined;
}

void CPropertyGrid::SetEmptyString(std::basic_string<TCHAR> strEmpty)
{
  m_strEmpty = strEmpty;
}
...
bool CPropertyGrid::CItem::operator==(const std::basic_string<TCHAR>& name) const
{
  return m_name == name;
}
...
HSECTION CPropertyGrid::AddSection(std::basic_string<TCHAR> title, bool collapsed, HSECTION after)
{
  // build it
  CSection section;
...
HITEM CPropertyGrid::AddItem(HSECTION hs, EItemType type, std::basic_string<TCHAR> name, void* pValue, bool editable, bool undefined, HITEM after)
{
  ...
  else if (type == IT_STRING || type == IT_TEXT || type == IT_FILE || type == IT_FOLDER) item.m_strValue = *(std::basic_string<TCHAR>*)pValue;
...
HITEM CPropertyGrid::AddCustomItem(HSECTION section, std::basic_string<TCHAR> name, ICustomItem* pItem, bool editable, HITEM after)
{
  pItem->m_pGrid = this;
  return AddItem(section, IT_CUSTOM, name, pItem, editable, false, after);
}

HITEM CPropertyGrid::AddStringItem(HSECTION section, std::basic_string<TCHAR> name, std::basic_string<TCHAR> value, bool editable, HITEM after)
{
  return AddItem(section, IT_STRING, name, &value, editable, false, after);
}

HITEM CPropertyGrid::AddTextItem(HSECTION section, std::basic_string<TCHAR> name, std::basic_string<TCHAR> value, bool editable, HITEM after)
{
  return AddItem(section, IT_TEXT, name, &value, editable, false, after);
}

HITEM CPropertyGrid::AddIntegerItem(HSECTION section, std::basic_string<TCHAR> name, int value, std::basic_string<TCHAR> format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddDoubleItem(HSECTION section, std::basic_string<TCHAR> name, double value, std::basic_string<TCHAR> format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddBoolItem(HSECTION section, std::basic_string<TCHAR> name, bool value, bool editable, bool undefined, HITEM after)
{
  return AddItem(section, IT_BOOLEAN, name, &value, editable, undefined, after);
}

HITEM CPropertyGrid::AddDateItem(HSECTION section, std::basic_string<TCHAR> name, COleDateTime value, std::basic_string<TCHAR> format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddDateTimeItem(HSECTION section, std::basic_string<TCHAR> name, COleDateTime value, std::basic_string<TCHAR> format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddFileItem(HSECTION section, std::basic_string<TCHAR> name, std::basic_string<TCHAR> value, std::basic_string<TCHAR> filter, bool editable, HITEM after)
{
...
HITEM CPropertyGrid::AddFolderItem(HSECTION section, std::basic_string<TCHAR> name, std::basic_string<TCHAR> value, std::basic_string<TCHAR> title, bool editable, HITEM after)
{
...
HITEM CPropertyGrid::AddColorItem(HSECTION section, std::basic_string<TCHAR> name, COLORREF value, bool editable, bool undefined, HITEM after)
{
  return AddItem(section, IT_COLOR, name, &value, editable, undefined, after);
}

HITEM CPropertyGrid::AddFontItem(HSECTION section, std::basic_string<TCHAR> name, LOGFONT value, bool editable, bool undefined, HITEM after)
{
...
bool CPropertyGrid::GetItemValue(HITEM item, std::basic_string<TCHAR>& strValue) const
{
  // get the item
...
bool CPropertyGrid::SetItemValue(HITEM item, const std::basic_string<TCHAR> strValue)
{
  // get the item
  CItem* pItem = FindItem(item);
  if (pItem == NULL) return false;
...
void CPropertyGrid::InitControl()
{
  ...
  if (m_strCancel.empty())
  {
    ...
    if (GetParent() && GetParent()->GetDlgItem(IDCANCEL))
    {
      ...
      m_strCancel = (LPTSTR)(LPCTSTR)strCancel; // std::basic_string<TCHAR>(CT2CA(strCancel.operator LPCWSTR()));
    }
  }
}
...
void FillGDIGradientRectHmin(CDC *pDc, const RECT &rect, COLORREF clrLeft, COLORREF clrRight)
{
	TRIVERTEX        vert[2] ;
	GRADIENT_RECT    gRect;
	vert [0] .x      = rect.left;
...
void CPropertyGrid::OnPaint()
{
  ...
  FillGDIGradientRectHmin(&dcMem, rc, m_clrBack, RGB(255,255,255));
  ...
  if (m_sections.empty())
  {
	...
	CString str(m_strEmpty.c_str());
    dcMem.DrawText(str, rect, DT_CENTER|DT_WORDBREAK|DT_NOPREFIX);
  }
  else
  {
    // needed
    ...
    else
    {
      ...
      for (vector<CSection>::iterator it = m_sections.begin(); it != m_sections.end(); ++it)
      {
        ...
        if (m_display_mode == DM_CATEGORIZED && !rcInter.IsRectEmpty())
        {
          ...
          {
            CSize sz = dcMem.GetTextExtent((LPCTSTR)it->m_title.c_str());
          ...
          dcMem.DrawText((LPCTSTR)it->m_title.c_str(), CRect(title_left+GetTextMargin(), title_top, w, title_top+m_line_height), DT_END_ELLIPSIS|DT_LEFT|DT_SINGLELINE|DT_VCENTER|DT_NOPREFIX); 
        }
...
void CPropertyGrid::DrawItem(CDC& dc, int w, int x, int y, vector<CItem>::iterator& it)
{
  ...
  dc.DrawText((LPCTSTR)it->m_name.c_str(), -1, CRect(name_left, y, name_right, y+m_line_height), DT_END_ELLIPSIS|DT_LEFT|DT_SINGLELINE|DT_VCENTER|DT_NOPREFIX);
  ...
  else
  {
    ...
    // now draw text
    std::basic_string<TCHAR> strValue = it->m_strValue;
    switch (it->m_type)
    {
    case IT_TEXT:
      {
        size_t j;
        while (( j = strValue.find( _T("\r\n") )) != std::basic_string<TCHAR>::npos)
    ...
    case IT_INTEGER:
      {
        CString strTemp;
        std::basic_string<TCHAR> strFormat = _T("%d");
        if (it->m_options.size() && !it->m_options.front().empty()) strFormat = it->m_options.front();
        strTemp.Format((LPCTSTR)strFormat.c_str(), it->m_nValue);
        strValue = (LPTSTR)(LPCTSTR)(strTemp);
    ...
    case IT_DOUBLE:
      {
        CString strTemp;
        std::basic_string<TCHAR> strFormat = _T("%g");
        if (it->m_options.size() && !it->m_options.front().empty()) strFormat = it->m_options.front();
        strTemp.Format((LPCTSTR)strFormat.c_str(), it->m_dValue);
        strValue = (LPTSTR)LPCTSTR(strTemp);
    ...
    case IT_DATE:
      {
        CString strTemp;
        if (it->m_options.size() && !it->m_options.front().empty()) strTemp = it->m_dtValue.Format((LPCTSTR)it->m_options.front().c_str());
    ...
    case IT_FOLDER:
      {
        TCHAR szBuffer[1024];
        _tcscpy(szBuffer, (LPCTSTR)strValue.c_str());
        PathCompactPath(dc.GetSafeHdc(), szBuffer, rcValue.Width());
        strValue = std::basic_string<TCHAR>(szBuffer);
        break;
      }
    ...
    dc.DrawText((LPCTSTR)strValue.c_str(), -1, rcValue, DT_END_ELLIPSIS|DT_LEFT|DT_SINGLELINE|DT_VCENTER|DT_NOPREFIX);
  }
...
LRESULT CPropertyGrid::OnEditChanged(WPARAM wParam, LPARAM lParam)
{
  ...
  if (pItem)
  {
    ...
    {
      if (pItem->m_pCustom->OnItemEdited((std::basic_string<TCHAR>&)(wParam)))
        GetOwner()->SendMessage(WM_PG_ITEMCHANGED, pItem->m_id);
      DeleteEditControl();
      Invalidate();
    }
...
void CPropertyGrid::EditFocusedItem()
{
  ...
  if (pItem)
  {
    ...
    if (pItem->m_type == IT_TEXT)
    {
      ...
      CString strValue = (LPCTSTR)pItem->m_strValue.c_str();
      dlg.AddDlgControl(_T("EDIT"), (LPCTSTR)pItem->m_strValue.c_str(), STYLE_EDIT|WS_VSCROLL|WS_HSCROLL|ES_AUTOHSCROLL|ES_AUTOVSCROLL|ES_LEFT|ES_MULTILINE|ES_WANTRETURN, EXSTYLE_EDIT, CRect(7, 7, 200, 100), (void*) &strValue);
      dlg.AddDlgControl(_T("BUTTON"), (LPCTSTR)m_strOk.c_str(), STYLE_BUTTON, EXSTYLE_BUTTON, CRect(56, 106, 106, 120), NULL, IDOK); 
      dlg.AddDlgControl(_T("BUTTON"), (LPCTSTR)m_strCancel.c_str(), STYLE_BUTTON, EXSTYLE_BUTTON, CRect(110, 106, 160, 120), NULL, IDCANCEL); 
      if (dlg.DoModal() == IDOK)
      {
        pItem->m_strValue = (LPTSTR)LPCTSTR(strValue);
        pItem->m_undefined = false;
        GetOwner()->SendMessage(WM_PG_ITEMCHANGED, pItem->m_id);
        Invalidate();
      }
    ...
    else if (pItem->m_type == IT_DATETIME)
    {
      ...
      dlg.AddDlgControl(_T("STATIC"), (LPCTSTR)m_strDate.c_str(), STYLE_STATIC, EXSTYLE_STATIC, CRect(7, 3, 60, 12));
      dlg.AddDlgControl(_T("STATIC"), (LPCTSTR)m_strTime.c_str(), STYLE_STATIC, EXSTYLE_STATIC, CRect(67, 3, 120, 12));
      ...
      dlg.AddDlgControl(_T("BUTTON"), (LPCTSTR)m_strOk.c_str(), STYLE_BUTTON, EXSTYLE_BUTTON, CRect(7, 37, 60, 51), NULL, IDOK); 
      dlg.AddDlgControl(_T("BUTTON"), (LPCTSTR)m_strCancel.c_str(), STYLE_BUTTON, EXSTYLE_BUTTON, CRect(67, 37, 120, 51), NULL, IDCANCEL); 
      if (dlg.DoModal() == IDOK)
      {
    ...
    {
      CFileDialog dlg(TRUE, NULL, (LPCTSTR)pItem->m_strValue.c_str(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, (LPCTSTR)pItem->m_options.front().c_str(), GetParent());
      if (dlg.DoModal() == IDOK)
      {
        pItem->m_strValue = std::basic_string<TCHAR>(dlg.GetPathName());
        pItem->m_undefined = false;
        GetOwner()->SendMessage(WM_PG_ITEMCHANGED, pItem->m_id);
        Invalidate();
      }
